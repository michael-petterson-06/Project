...
const sendMarsTemperature = (resposta, resposta2) => {
  let sucessoOuErro = Math.random() <= 0.6;
  const temperatura = getMarsTemperature();
    if (sucessoOuErro)  setTimeout(() => resposta(temperatura), messageDelay());
   else setTimeout(() => resposta2('Robot is busy'), messageDelay());
}
